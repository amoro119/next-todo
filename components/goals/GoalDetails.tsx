          ...
          {!sortedTodos.length ? (
            ...
            <div className="space-y-2" role="list" aria-label="目标任务">
              {sortedTodos.map((todo) => (
                <div key={todo.id} role="listitem" draggable onDragStart={() => setDraggedId(todo.id)} onDragEnd={() => { setDraggedId(null); setDragOverId(null) }} onDragOver={(event) => { event.preventDefault(); setDragOverId(todo.id) }} onDrop={() => handleDrop(todo.id)} className={`${TODO_ITEM_STYLES.row} ${todo.completed ? TODO_ITEM_STYLES.completedRow : ''} cursor-grab active:cursor-grabbing ${dragOverId === todo.id ? 'border-foreground bg-muted/50' : ''} ${draggedId === todo.id ? 'opacity-50' : ''}`}>
                  <div className={`${TODO_ITEM_STYLES.goalContent}${todo.completed ? ` ${TODO_ITEM_STYLES.completedContent}` : ''}`}>
                    <span className="hidden shrink-0 cursor-grab text-muted-foreground sm:inline active:cursor-grabbing" aria-hidden="true"><GripVertical className="h-4 w-4" /></span>
                    <button type="button" role="checkbox" aria-checked={todo.completed} aria-label={todo.completed ? `标记 ${todo.title} 为未完成` : `标记 ${todo.title} 为完成`} onClick={() => toggleTodo(todo)} className={`${TODO_ITEM_STYLES.checkbox} ${todo.completed ? TODO_ITEM_STYLES.checkboxCompleted : TODO_ITEM_STYLES.checkboxPending}`}>{todo.completed && <Check className="h-3.5 w-3.5 text-[oklch(var(--muted-foreground))]" aria-hidden="true" />}</button>
                    <button type="button" className={`${TODO_ITEM_STYLES.title} min-w-0 cursor-pointer truncate text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${todo.completed ? TODO_ITEM_STYLES.completedTitle : ''}`} onClick={() => setEditingTask(todo)}>{todo.title}</button>
                    {todo.due_date && <span className={`${todo.completed ? TODO_ITEM_STYLES.completedMeta : TODO_ITEM_STYLES.meta} shrink-0`}>{formatDate(todo.due_date, { month: 'short', day: 'numeric' })}</span>}
