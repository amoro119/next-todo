...
Read the canonical workflow at `../../../skills/next-todo/SKILL.md` completely before acting. Use the same API contract, action rules, safety requirements, and verification criteria defined there.

## Codex runtime adaptations

1. Resolve `../../../skills/next-todo/scripts/todo-api.sh` relative to this file and run that absolute path. Do not use the Hermes-only `${HERMES_SKILL_DIR}` token.
